...
export class LessonsService {
  ...
  async completeLesson(telegramId: string): Promise<void> {
    ...
    await this.lessonsRepository.update(
      { id: lastLesson.id },
      { status: LessonStatus.COMPLETED, updatedAt: () => 'CURRENT_TIMESTAMP' },
    );
  }
  ...
  async cancelDraft(telegramId: string): Promise<void> {
    const user = await this.usersService.getUserByTelegramId(telegramId);
    if (!user) return;
  ...
  async cancelLesson(telegramId: string): Promise<void> {
    const lastLesson = await this.getLastLesson(telegramId);
